...
By default, an alarm retries after `abort` resets the Durable Object. Pass `{ retryAlarm: false }` to stop the current alarm from running again. `retryAlarm` does nothing outside an alarm handler.
...
- An optional `DurableObjectAbortOptions` object:
  - `retryAlarm` <Type text="boolean" />: Set this to `false` to stop the current alarm from retrying. It defaults to `true` and does nothing outside an alarm handler.
